...
await test("each partner on the website's strip has researched facts behind it", () => {
  for (const item of INTEGRATIONS) {
    ...
    if (!id) continue;
    // Calendly is being built on another branch and brings its own entry with
    // it. Everything else must already be answered for.
    if (id === ("calendly" as PartnerId)) continue;
