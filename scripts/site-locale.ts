 ...
 * its reader an Impressum naming a real company (§5 DDG). So `SITE_GERMAN=off`
 * leaves them out of the build (scripts/build-site.ts), and the picker, the
 * alternates and the links must offer exactly what was built — a picker that
 * links /de-de on a build without /de-de sends people to a 404, and an
 * hreflang that names a missing page tells search engines it exists.
 *
 * Not `language.de`: that flag says whether Belline answers in German, which
 * is a different question from whether we publish a German page.
 */
export function publishedCountries(env: Env = process.env): readonly CountryPage[] {
...
export function renderLocalePicker(current: SiteCountry, countries: readonly CountryPage[] = publishedCountries()): string {
  const de = current !== "AE";
  const words = de
...
export function applyLocalePicker(html: string, current: SiteCountry, countries?: readonly CountryPage[]): string {
  const from = html.indexOf(START);
  const to = html.indexOf(END);
  if (from < 0 || to < 0 || to < from) throw new Error("A landing page has lost its locale:start / locale:end markers.");
  return `${html.slice(0, from)}${renderLocalePicker(current, countries)}${html.slice(to + END.length)}`;
}
