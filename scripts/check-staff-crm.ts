...
await test("the timeline hook: an outside event lands on the lead, and a watched demo makes it hot", async () => {
  ...
  // The lead page leaves the slot for the video demo, and the slot renders nothing yet.
  const page = source("src/app/(internal)/sales/leads/[id]/page.tsx");
  assert.match(page, /id="video-demo"[\s\S]{0,120}<VideoDemoSlot lead=\{lead\} \/>/);
  assert.match(code(source("src/app/(internal)/sales/leads/[id]/VideoDemoSlot.tsx")), /return null;/);
});
