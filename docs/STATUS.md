...
At full use on the conservative UAE basis the plans now land **Starter 41.7%, Growth 31.3%, Scale 31.2%** — over the floor, but by about a point, not comfortably. Voice is what eats it: Scale's 600 minutes cost $63.42 against $217.56 of revenue before a single conversation, the number rental or the card fee. That is the real reason the voice allowance did not move, and should not until it is measured.

**Known exposure, not a hypothetical:** an owner can switch their venue to Sonnet on the Agent page (`AgentEditor.tsx` offers it), which doubles a text conversation from $0.0148 to $0.0296 and would put Scale near 21% at full use — under the floor. `check:plans` ties the conservative basis to the seeded model, so the assumption cannot rot silently, but no test can see what a customer chooses. If venues start moving to Sonnet, the allowances are what has to give.
