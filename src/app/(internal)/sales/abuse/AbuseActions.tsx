...
/** Allow, note or suspend one flagged signup. */
  ...
  return (
    <div style={{ display: "grid", gap: 6, minWidth: 220 }}>
      <textarea
        ...
        placeholder="Note (optional for allow and suspend)"
        aria-label="Note"
        rows={2}
        maxLength={1000}
      />
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {status !== "allowed" && (
          <button className="btn btn-accent" type="button" disabled={busy} onClick={() => void act("allow")}>
            Allow
          </button>
        )}
        <button className="btn" type="button" disabled={busy || !note.trim()} onClick={() => void act("note")}>
          Note
        </button>
        {canSuspend && status !== "suspended" && (
          <button className="btn" type="button" disabled={busy} onClick={() => void act("suspend")}>
            Suspend trial
          </button>
        )}
        {canSuspend && status === "suspended" && (
          <button className="btn" type="button" disabled={busy} onClick={() => void act("unsuspend")}>
            Lift suspension
          </button>
        )}
      </div>
...
export function VerifyButton({ userId }: { userId: string }) {
  ...
  return (
    <div>
      ...
      {error && (
        <p role="alert" style={{ margin: "4px 0 0", fontSize: 12, color: "var(--bad)" }}>
          {error}
        </p>
      )}
    </div>
