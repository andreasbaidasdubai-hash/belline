 ...
 *
 * The checkout also asks what the business does, and the answer has to mean
 * the same thing to the form, to the server, to the words the agent uses and
 * to a report months later. So the list the customer sees, the engine each
 * option runs on, and the aliases old links still arrive with all live here
 * too, and check:signup fails if those drift.
 *
