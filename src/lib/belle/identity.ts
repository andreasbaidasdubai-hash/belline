 ...
 * customer (read-only)? Read loosely so it works before and after the staff
 * console's `Session.viewAs` exists on this branch.
 */
export function isViewAs(session: Session | undefined | null): boolean {
  return Boolean(session && (session as Session & { viewAs?: unknown }).viewAs);
}
